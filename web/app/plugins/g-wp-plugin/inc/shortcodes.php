<?php
		...
		function youtube_handlerbk( $atts, $content = null ) {
			...
			if (strlen($content) > 4 ){
				return  '<iframe  title="YouTube video player" width="500" height="390" src="http://www.youtube.com/embed/'.$content .'" frameborder="0" allowfullscreen></iframe>';
			}else{
				return "[youtube]";
			}
		}
		...
		function recent_posts_function() {
           ...
           if (have_posts()) :
              while (have_posts()) : the_post();
                 $return_string = '<li><a href="'.get_permalink().'">'.get_the_title().'</li></a>';
              endwhile;
           endif;
           wp_reset_query();
